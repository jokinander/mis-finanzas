...
export default function App() {
  ...
  return (
    <div style={S.root}>
      ...
      <div style={S.nav}>{[["dashboard", "Resumen"], ["transactions", "Movimientos"], ["chiches", "Chiches"], ["annual", "Anual"], ["insights", "Análisis"]].map(([k, l]) => (<button key={k} onClick={() => setView(k)} style={{ ...S.nb, ...(view === k ? S.nbA : {}) }}>{l}</button>))}</div>
      ...
      {view === "annual" && (
        <div style={{ animation: "fadeIn .4s" }}>
          {/* Tabla ARS */}
          <p style={{ fontSize: 12, color: "#64748b", fontWeight: 600, textTransform: "uppercase", letterSpacing: .5, marginBottom: 8 }}>🇦🇷 Pesos (ARS) — {selYear}</p>
          ...
          {cumulData.some(d => d.ingUSD > 0 || d.egUSD > 0) && (<>
            <p style={{ fontSize: 12, color: "#64748b", fontWeight: 600, textTransform: "uppercase", letterSpacing: .5, marginBottom: 8 }}>🇺🇸 Dólares (USD) — {selYear}</p>
      ...
      {view === "insights" && (() => {
        ...
        <div style={{ animation: "fadeIn .4s" }}>
          ...
          {/* Meta 20% sobre egresos promedio */}
          {(() => {
            // Promedio de egresos ARS de meses anteriores CON datos (excluye el mes actual)
            const prevMonthsARS = [], prevMonthsUSD = [];
            for (let i = 0; i < 12; i++) {
              const mi = selMonth - 1 - i;
              const yi = mi < 0 ? selYear - 1 : selYear;
              const mIdx = mi < 0 ? mi + 12 : mi;
              const mt = txs.filter(t => { const d = new Date(t.date + "T12:00:00"); return d.getFullYear() === yi && d.getMonth() === mIdx; });
              const eA = sumBy(mt, "egreso", "ARS"), eU = sumBy(mt, "egreso", "USD");
              if (eA > 0) prevMonthsARS.push(eA);
              if (eU > 0) prevMonthsUSD.push(eU);
            }
            const avgEgARS = prevMonthsARS.length > 0 ? prevMonthsARS.reduce((s, v) => s + v, 0) / prevMonthsARS.length : 0;
            const avgEgUSD = prevMonthsUSD.length > 0 ? prevMonthsUSD.reduce((s, v) => s + v, 0) / prevMonthsUSD.length : 0;
            const metaARS = avgEgARS * 1.2;
            const metaUSD = avgEgUSD * 1.2;
            const faltaARS = metaARS - ingARS;
            const faltaUSD = metaUSD - ingUSD;
            return (
              <div style={{ ...S.insC, borderLeft: "4px solid #10b981", marginBottom: 12 }}>
                <span style={{ fontSize: 22 }}>🎯</span>
                <div style={{ fontSize: 14, color: "#cbd5e1", lineHeight: 1.8, width: "100%" }}>
                  <p style={{ fontWeight: 700, color: "#e2e8f0", marginBottom: 6 }}>Meta: ingresos 20% arriba de tus egresos</p>
                  {avgEgARS === 0 && avgEgUSD === 0 ? (
                    <p>Todavía no hay egresos de meses anteriores para calcular el promedio.</p>
                  ) : (<>
                    {avgEgARS > 0 && (
                      <div style={{ marginBottom: 10 }}>
                        <p style={{ color: "#94a3b8" }}>🇦🇷 Pesos — promedio de egresos: <strong style={{ color: "#e2e8f0" }}>{fmt(avgEgARS)}</strong></p>
                        <p style={{ color: "#94a3b8" }}>Meta de ingresos (×1,2): <strong style={{ color: "#e2e8f0" }}>{fmt(metaARS)}</strong></p>
                        <p style={{ color: "#94a3b8" }}>Ingresos de este mes: <strong style={{ color: "#10b981" }}>{fmt(ingARS)}</strong></p>
                        {faltaARS <= 0
                          ? <p style={{ color: "#10b981", fontWeight: 700 }}>✅ ¡Meta cumplida! Te sobran {fmt(Math.abs(faltaARS))} por encima de la meta.</p>
                          : <p style={{ color: "#f59e0b", fontWeight: 700 }}>Te faltan {fmt(faltaARS)} para llegar a la meta.</p>}
                      </div>
                    )}
                    {avgEgUSD > 0 && (
                      <div style={{ borderTop: avgEgARS > 0 ? "1px solid #334155" : "none", paddingTop: avgEgARS > 0 ? 8 : 0 }}>
                        <p style={{ color: "#94a3b8" }}>🇺🇸 Dólares — promedio de egresos: <strong style={{ color: "#e2e8f0" }}>{fmt(avgEgUSD, "USD")}</strong></p>
                        <p style={{ color: "#94a3b8" }}>Meta de ingresos (×1,2): <strong style={{ color: "#e2e8f0" }}>{fmt(metaUSD, "USD")}</strong></p>
                        <p style={{ color: "#94a3b8" }}>Ingresos de este mes: <strong style={{ color: "#3b82f6" }}>{fmt(ingUSD, "USD")}</strong></p>
                        {faltaUSD <= 0
                          ? <p style={{ color: "#10b981", fontWeight: 700 }}>✅ ¡Meta cumplida! Te sobran {fmt(Math.abs(faltaUSD), "USD")} por encima de la meta.</p>
                          : <p style={{ color: "#f59e0b", fontWeight: 700 }}>Te faltan {fmt(faltaUSD, "USD")} para llegar a la meta.</p>}
                      </div>
                    )}
                    <p style={{ color: "#64748b", fontSize: 12, marginTop: 8 }}>Promedio calculado sobre {Math.max(prevMonthsARS.length, prevMonthsUSD.length)} mes(es) con egresos cargados.</p>
                  </>)}
                </div>
              </div>
            );
      ...
      {view === "chiches" && (() => {
        ...
        return (
          <div style={{ animation: "fadeIn .4s" }}>
            <h3 style={{ ...S.secT, marginBottom: 16 }}>🎁 Chiches</h3>

            {/* IPC acumulado 3 meses */}
            <div style={{ ...S.insC, borderLeft: "4px solid #f59e0b", marginBottom: 12 }}>
              <span style={{ fontSize: 22 }}>📈</span>
              <div style={{ fontSize: 14, color: "#cbd5e1", lineHeight: 1.8, width: "100%" }}>
                <p style={{ fontWeight: 700, color: "#e2e8f0", marginBottom: 6 }}>IPC acumulado — últimos 3 meses</p>
                {ult3.length === 0 ? (
                  <p>Cargando datos de inflación... {ecoData.loading ? "" : "(no disponibles)"}</p>
                ) : (<>
                  <p style={{ fontSize: 28, fontWeight: 800, color: "#fbbf24", fontFamily: "'Fraunces',serif", margin: "4px 0" }}>{ipcAcum.toFixed(1)}%</p>
                  <p style={{ color: "#94a3b8", fontSize: 13 }}>Esto es lo que aumentaron los precios en total en el período. Un buen referente para pedir aumentos.</p>
                  <div style={{ marginTop: 8, borderTop: "1px solid #334155", paddingTop: 8 }}>
                    {ult3.map(m => (
                      <p key={m.fecha} style={{ color: "#94a3b8", fontSize: 13 }}>
                        {new Date(m.fecha + "T12:00:00").toLocaleDateString("es-AR", { month: "long", year: "numeric" })}: <strong style={{ color: "#e2e8f0" }}>{m.valor}%</strong>
                      </p>
                    ))}
                  </div>
                </>)}
