    ...
    <div style={S.root}>
      ...
      {showForm && (<div style={S.ov} onClick={() => setShowForm(false)}><div style={{ ...S.mod, animation: "popIn .3s" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          {<div style={{ flex: 1, marginBottom: 14 }}><label style={S.fLbl}>Tipo</label><select style={S.fIn} value={form.tipoIngreso} onChange={e => setForm({ ...form, tipoIngreso: e.target.value })}><option value="Fijo">Fijo</option><option value="Variable">Variable</option></select></div>><option value="Fijo">Fijo</option><option value="Variable">Variable</option></select></div>
          )}
        </div>
